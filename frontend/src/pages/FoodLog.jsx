...
import { Save, ChevronLeft, ChevronRight, Calendar, BookOpen, BarChart2, ArrowLeft, Utensils } from "lucide-react";
...
const Page = React.forwardRef(({ children, isLeft }, ref) => (
  <div ref={ref} data-density="soft" style={{
    ...
    <div style={{
      ...
      backgroundImage: "repeating-linear-gradient(transparent, transparent 31px, rgba(74, 222, 128, 0.1) 31px, rgba(74, 222, 128, 0.1) 32px)",
...
const YearBook = ({ year, logCount, onClick }) => (
  <div onClick={onClick} style={{
    ...
    boxShadow: "4px 4px 14px rgba(0,0,0,0.8), inset -3px 0 8px rgba(74,222,128,0.2)",
    transition: "transform 0.2s, box-shadow 0.2s", border: "1px solid rgba(74,222,128,0.3)"
  }}
    onMouseEnter={e => { e.currentTarget.style.transform = "translateY(-10px)"; e.currentTarget.style.boxShadow = "0 0 25px rgba(74,222,128,0.4), inset -3px 0 8px rgba(74,222,128,0.4)"; }}
    onMouseLeave={e => { e.currentTarget.style.transform = "none"; e.currentTarget.style.boxShadow = "4px 4px 14px rgba(0,0,0,0.8), inset -3px 0 8px rgba(74,222,128,0.2)"; }}
  >
    <span style={{ writingMode: "vertical-rl", transform: "rotate(180deg)", color: "#4ade80", fontWeight: "bold", fontSize: 18, letterSpacing: 4, textShadow: "0 0 10px rgba(74,222,128,0.5)" }}>{year}</span>
    <span style={{ color: "rgba(74,222,128,0.6)", fontSize: 11, fontFamily: "monospace" }}>{logCount} logs</span>
  </div>
);
...
const NavBtn = ({ onClick, children, active }) => (
  <button onClick={onClick} style={{
    ...
    border: `1px solid ${active ? "#4ade80" : "#27272a"}`,
...
const DiaryView = ({ logs, month, year, onBack, currentDate, onSaved }) => {
  ...
  const [aiData, setAiData] = useState({ 
    summary: "Save your log to get AI nutrition synthesis.", 
    metrics: { proteinGrams: 0, calories: 0 } 
  });
  ...
  const editor = useEditor({
    ...
    content: `<h2>Nutrition Log: ${currentDate.toLocaleDateString()}</h2><h3>Macros</h3><ul><li><strong>Protein:</strong> [ ]g</li><li><strong>Estimated Calories:</strong> [ ]</li></ul><h3>Meals</h3><ul><li><strong>Breakfast:</strong> </li><li><strong>Lunch:</strong> </li><li><strong>Dinner:</strong> </li></ul>`,
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const response = await fetch("http://localhost:5000/api/lifelog", {
        ...
        body: JSON.stringify({ textContent: editor.getText(), date: new Date().toISOString(), logType: "food" })
      });
  ...
  const metricBox = (label, val) => (
    <div key={label} style={{ background: "rgba(74,222,128,0.05)", borderRadius: 8, padding: "12px 8px", textAlign: "center", border: "1px solid rgba(74,222,128,0.2)" }}>
  ...
  return (
    <div style={{ 
      ...
      <style>{`
        ...
        .notebook-editor h3 { font-size: 14px; font-weight: 600; line-height: 32px; margin: 0; color: #4ade80; }
        ...
        .ai-synthesis-content h3 { font-size: 14px; font-weight: 700; line-height: 32px; margin: 0; color: #4ade80; text-transform: uppercase; letter-spacing: 1px; }
          ...
          {[
            ...(isCurrentMonth ? [
              <Page key="today-l" isLeft>
                <div style={{ borderBottom: "2px solid #4ade80", marginBottom: 12, paddingBottom: 8, display: "flex", justifyContent: "space-between", alignItems: "center", height: "32px", boxSizing: "border-box" }}>
                  <span style={{ color: "#4ade80", fontSize: 11, letterSpacing: 2, textTransform: "uppercase", fontWeight: "600", lineHeight: "16px" }}>Today</span>
                ...
                <button 
                  ...
                  style={{ 
                    ...
                    background: "#16a34a", color: "#ffffff", border: "none", borderRadius: 8, padding: "8px 16px", 
                    cursor: "pointer", fontSize: 13, fontWeight: "600", boxShadow: "0 4px 12px rgba(22,163,74,0.3)",
              ...
              <Page key="today-r">
                ...
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8, marginTop: "auto" }}>
                  {metricBox("Protein", `${aiData.metrics?.proteinGrams || 0}g`)}
                  {metricBox("Calories", aiData.metrics?.calories || 0)}
                </div>
                {logs.length > 0 && (
                   <div style={{ marginTop: 16, textAlign: "center", fontSize: 11, color: "#52525b" }}>← Turn page for past meals →</div>
                )}
              </Page>
            ] : []),
            ...
            ...logs.flatMap(log => [
              <Page key={`${log.id}-L`} isLeft>
                <div style={{ borderBottom: "2px solid #27272a", marginBottom: 12, paddingBottom: 8, display: "flex", justifyContent: "space-between", height: "32px", boxSizing: "border-box" }}>
                  <span style={{ color: "#71717a", fontSize: 11, letterSpacing: 2, textTransform: "uppercase", fontWeight: "600", lineHeight: "16px" }}>Meal Log</span>
              ...
              <Page key={`${log.id}-R`}>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8, marginTop: "auto" }}>
                  {metricBox("Protein", `${log.metrics?.proteinGrams || 0}g`)}
                  {metricBox("Calories", log.metrics?.calories || 0)}
                </div>
              </Page>
            ])
          ]}
        ...
        {MONTHS.map((name, i) => {
          ...
          return (
            <div key={i} style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 8 }}>
              ...
              <span style={{ fontSize: 9, color: data ? "#4ade80" : "#52525b", fontFamily: "monospace", letterSpacing: 1 }}>{name.slice(0,3).toUpperCase()}</span>
            </div>
          );
        })}
      </div>
...
const YearShelf = ({ years, onSelectYear }) => (
  <div style={{ display: "flex", flexDirection: "column", alignItems: "center", width: "100%" }}>
    <div style={{ marginBottom: 32, textAlign: "center" }}>
      <h2 style={{ color: "#f4f4f5", fontSize: 28, margin: 0, letterSpacing: 1, fontWeight: "700" }}>Nutrition Archive</h2>
      <p style={{ color: "#a1a1aa", fontSize: 14, marginTop: 6 }}>Select a year to browse monthly meal diaries</p>
    </div>
...
const FoodLog = () => {
  ...
  const fetchHistory = useCallback(async () => {
    ...
    try {
      ...
      const response = await fetch("http://localhost:5000/api/lifelog/food", {
    ...
    <div style={{ minHeight: "100%", background: "#09090b", display: "flex", flexDirection: "column", alignItems: "center", padding: "40px 20px", fontFamily: "Inter, sans-serif" }}>
      <div style={{ width: "100%", maxWidth: 900, display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 40 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
          <div style={{ width: 40, height: 40, background: "rgba(74,222,128,0.1)", border: "1px solid rgba(74,222,128,0.3)", borderRadius: "10px", display: "flex", alignItems: "center", justifyContent: "center" }}>
            <Utensils size={20} color="#4ade80" />
          </div>
          <span style={{ color: "#f4f4f5", fontSize: 20, fontWeight: "700", letterSpacing: 1 }}>Nutrition Log</span>
        </div>
