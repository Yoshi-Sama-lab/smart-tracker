...
import { Save, ChevronLeft, ChevronRight, Calendar, BookOpen, BarChart2, ArrowLeft } from "lucide-react";
...
const Page = React.forwardRef(({ children, isLeft }, ref) => (
  <div ref={ref} data-density="soft" style={{
    ...
    <div style={{
      ...
      backgroundImage: "repeating-linear-gradient(transparent, transparent 31px, rgba(124, 58, 237, 0.1) 31px, rgba(124, 58, 237, 0.1) 32px)",
...
const YearBook = ({ year, logCount, onClick }) => (
  <div onClick={onClick} style={{
    ...
    boxShadow: "4px 4px 14px rgba(0,0,0,0.8), inset -3px 0 8px rgba(124,58,237,0.2)",
    transition: "transform 0.2s, box-shadow 0.2s", border: "1px solid rgba(124,58,237,0.3)"
  }}
    onMouseEnter={e => { e.currentTarget.style.transform = "translateY(-10px)"; e.currentTarget.style.boxShadow = "0 0 25px rgba(124,58,237,0.4), inset -3px 0 8px rgba(124,58,237,0.4)"; }}
    onMouseLeave={e => { e.currentTarget.style.transform = "none"; e.currentTarget.style.boxShadow = "4px 4px 14px rgba(0,0,0,0.8), inset -3px 0 8px rgba(124,58,237,0.2)"; }}
  >
    <span style={{ writingMode: "vertical-rl", transform: "rotate(180deg)", color: "#7c3aed", fontWeight: "bold", fontSize: 18, letterSpacing: 4, textShadow: "0 0 10px rgba(124,58,237,0.5)" }}>{year}</span>
    <span style={{ color: "rgba(124,58,237,0.6)", fontSize: 11, fontFamily: "monospace" }}>{logCount} logs</span>
  </div>
);
...
const DiaryView = ({ logs, month, year, onBack, currentDate, onSaved }) => {
  ...
  const [aiData, setAiData] = useState({ 
    summary: "Save your log to get your Coach's Synthesis.", 
    metrics: { steps: 0, setsCompleted: 0, dropSets: 0 } 
  });
  ...
  const editor = useEditor({
    ...
    content: `<h2>Training Log: ${currentDate.toLocaleDateString()}</h2><h3>Shoulder Focus</h3><ul><li><strong>Lateral Deltoids (Plates/Reps):</strong> </li><li><strong>Drop Sets / Volume:</strong> </li></ul><h3>Activity</h3><ul><li><strong>Daily Steps:</strong> [ ] / 15,000+ target</li></ul><p><em>Fatigue notes:</em> </p>`,
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const response = await fetch("http://localhost:5000/api/lifelog", {
        ...
        body: JSON.stringify({ textContent: editor.getText(), date: new Date().toISOString(), logType: "gym" })
      });
      const data = await response.json();
      
      if (data.success) {
        // Bulletproof mapping: Captures old schema, new Pydantic schema, or nested API responses
        setAiData({ 
          summary: data.aiSummary || data.coach_analysis_report || data.log?.aiSummary || "Synthesis complete.", 
          metrics: data.metrics || { steps: data.steps_taken || 0, setsCompleted: 0, dropSets: 0 } 
        });
        onSaved?.(); 
      }
  ...
  const metricBox = (label, val) => (
    <div key={label} style={{ background: "rgba(124,58,237,0.05)", borderRadius: 8, padding: "12px 8px", textAlign: "center", border: "1px solid rgba(124,58,237,0.2)" }}>
      <div style={{ fontSize: 10, letterSpacing: 1, color: "#a1a1aa", textTransform: "uppercase", marginBottom: 4, fontWeight: "600" }}>{label}</div>
      <div style={{ fontSize: 22, fontWeight: "bold", color: "#f4f4f5" }}>{val}</div>
    </div>
  );
  ...
  return (
    <div style={{ 
      ...
      <style>{`
        ...
        .notebook-editor h3 { font-size: 14px; font-weight: 600; line-height: 32px; margin: 0; color: #7c3aed; }
        ...
        .ai-synthesis-content h3 { font-size: 14px; font-weight: 700; line-height: 32px; margin: 0; color: #7c3aed; text-transform: uppercase; letter-spacing: 1px; }
      ...
      {/* Made the header slightly wider to accommodate the larger book */}
      ...
      <div style={{ background: "#09090b", borderRadius: 16, padding: "24px 28px", boxShadow: "0 30px 60px rgba(0,0,0,0.9)", display: "inline-block", border: "1px solid #27272a" }}>
        ...
        {/* FIX 3: Increased dimensions of the book! (Width 430 -> 480, Height 580 -> 640) */}
        <HTMLFlipBook 
          ...
          {[
            ...(isCurrentMonth ? [
              <Page key="today-l" isLeft>
                <div style={{ borderBottom: "2px solid #7c3aed", marginBottom: 12, paddingBottom: 8, display: "flex", justifyContent: "space-between", alignItems: "center", height: "32px", boxSizing: "border-box" }}>
                  <span style={{ color: "#7c3aed", fontSize: 11, letterSpacing: 2, textTransform: "uppercase", fontWeight: "600", lineHeight: "16px" }}>Today</span>
                ...
                <button 
                  ...
                  style={{ 
                    ...
                    background: "#7c3aed", color: "#ffffff", border: "none", borderRadius: 8, padding: "8px 16px", 
                    cursor: "pointer", fontSize: 13, fontWeight: "600", boxShadow: "0 4px 12px rgba(124,58,237,0.3)",
              ...
              <Page key="today-r">
                ...
                <div className="ai-synthesis-content" style={{ flex: 1, overflowY: "auto", userSelect: "text" }} onMouseDown={(e) => e.stopPropagation()} onMouseUp={(e) => e.stopPropagation()}>
                  <ReactMarkdown>{aiData.summary || "Generating synthesis..."}</ReactMarkdown>
                </div>

                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 8, marginTop: "auto" }}>
                  {metricBox("Steps", aiData.metrics?.steps?.toLocaleString() || 0)}
                  {metricBox("Sets", aiData.metrics?.setsCompleted || 0)}
                  {metricBox("Drop Sets", aiData.metrics?.dropSets || 0)}
                </div>
                {logs.length > 0 && (
                   <div style={{ marginTop: 16, textAlign: "center", fontSize: 11, color: "#52525b" }}>← Turn page for past sessions →</div>
                )}
              </Page>
            ] : []),
            ...
            ...logs.flatMap(log => [
              <Page key={`${log.id}-L`} isLeft>
                <div style={{ borderBottom: "2px solid #27272a", marginBottom: 12, paddingBottom: 8, display: "flex", justifyContent: "space-between", height: "32px", boxSizing: "border-box" }}>
                  <span style={{ color: "#71717a", fontSize: 11, letterSpacing: 2, textTransform: "uppercase", fontWeight: "600", lineHeight: "16px" }}>Session</span>
              ...
              <Page key={`${log.id}-R`}>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 8, marginTop: "auto" }}>
                  {metricBox("Steps", (log.steps_taken || log.metrics?.steps || 0).toLocaleString())}
                  {metricBox("Sets", log.metrics?.setsCompleted || "—")}
                  {metricBox("Drop Sets", log.metrics?.dropSets || "—")}
                </div>
              </Page>
            ])
          ]}
        ...
        {MONTHS.map((name, i) => {
          ...
          return (
            <div key={i} style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 8 }}>
              ...
              <span style={{ fontSize: 9, color: data ? "#7c3aed" : "#52525b", fontFamily: "monospace", letterSpacing: 1 }}>{name.slice(0,3).toUpperCase()}</span>
            </div>
          );
        })}
      </div>
    </div>
    <p style={{ color: "#71717a", fontSize: 12, marginTop: 16 }}>Click a book to open that month's diary</p>
  </div>
);
...
const YearShelf = ({ years, onSelectYear }) => (
  <div style={{ display: "flex", flexDirection: "column", alignItems: "center", width: "100%" }}>
    <div style={{ marginBottom: 32, textAlign: "center" }}>
      <h2 style={{ color: "#f4f4f5", fontSize: 28, margin: 0, letterSpacing: 1, fontWeight: "700" }}>Training Archive</h2>
      <p style={{ color: "#a1a1aa", fontSize: 14, marginTop: 6 }}>Select a year to browse monthly volumes</p>
    </div>
...
const GymLog = () => {
  ...
  const fetchHistory = useCallback(async () => {
    ...
    try {
      ...
      const response = await fetch("http://localhost:5000/api/lifelog/gym", {
    ...
    <div style={{ minHeight: "100%", background: "#09090b", display: "flex", flexDirection: "column", alignItems: "center", padding: "40px 20px", fontFamily: "Inter, sans-serif" }}>
      <div style={{ width: "100%", maxWidth: 900, display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 40 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
          <div style={{ width: 40, height: 40, background: "rgba(124,58,237,0.1)", border: "1px solid rgba(124,58,237,0.3)", borderRadius: "10px", display: "flex", alignItems: "center", justifyContent: "center" }}>
            <BookOpen size={20} color="#7c3aed" />
          </div>
          <span style={{ color: "#f4f4f5", fontSize: 20, fontWeight: "700", letterSpacing: 1 }}>Training Log</span>
        </div>
      ...
      {/* FIX 1: Keep the shelf rendered in the background even when the diary is open */}
      ...
      {/* The DiaryView will now render ON TOP of the shelf as a modal */}
      {view === "diary" && (
        <DiaryView 
